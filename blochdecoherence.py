...
def plotsphere(ax):
    ...
    ax.plot_surface(x,y,z,color="grey",shade="yes",alpha=0.05,linewidth=0.1,rstride=5,cstride=5)
    plotequator(ax)
...
def plotequatorialplane(ax):
    dr=.01
    dtheta=.05
    rpts=np.linspace(0,1,100)
    thetapts=np.linspace(0,2.*pi,100)

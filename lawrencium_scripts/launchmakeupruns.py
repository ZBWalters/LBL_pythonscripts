...
nodequeue=makenodequeue()#makenodequeue(nodefile)
...
dirlist=natural_sort(glob.glob("results*/*[0-9]/"))
zdiplist=glob.glob("results*/*[0-9]/ZD*exp*.Dat")#don't want to run inside directories that already contain ZDipoleexpect.Dat
zdiplist=map(dirname,zdiplist)
dirlist=list(set(dirlist)-set(zdiplist))
print("makeup runs needed in directories\t"+str(dirlist))
absnmax=len(dirlist)

...
def ellipticalpulse(wlas, wenv, I0, poltheta, polphase, CEPphase,tstart):
    #return an elliptically polarized pulse characterized by 
    #I=I0(cos(poltheta) zhat+ sin(theta) exp(I polphase) xhat)
    #as the sum of two linearly polarized pulses which can be written into the
    #input file appropriately by multiplepulses

    retwlaslist=[wlas, wlas]
    retwenvlist=[wenv, wenv]
    retI0list=[I0*cos(poltheta), I0*sin(poltheta)]
    retCEPlist=[CEPphase, CEPphase+polphase/pi]
    rettstartlist=[tstart, tstart]
    return retwlaslist, retwenvlist, retI0list, retCEPlist, rettstartlist

def ellipticalpulse_list(wlaslist, wenvlist, I0list, polthetalist, polphaselist,
                         CEPphaselist, tstartlist):
    retwlaslist=[]
    retwenvlist=[]
    retI0list=[]
    retCEPlist=[]
    rettstartlist=[]
    pulsethetalist=[]
    for i in range(len(wlaslist)):
        tmpwlaslist, tmpwenvlist, tmpI0list, tmpCEPlist, tmptstartlist=\
        ellipticalpulse(wlaslist[i], wenvlist[i], I0list[i], polthetalist[i],
                        polphaselist[i], CEPphaselist[i], tstartlist[i])
        retwlaslist=retwlaslist+tmpwlaslist
        retwenvlist=retwenvlist+tmpwenvlist
        retI0list=retI0list+tmpI0list
        retCEPlist=retCEPlist+tmpCEPlist
        rettstartlist=rettstartlist+tmptstartlist
        pulsethetalist=pulsethetalist+[0, pi/2]
    return retwlaslist, retwenvlist, retI0list, retCEPlist, rettstartlist,\
            pulsethetalist

def multiplepulses_ellipticalpulse(masterscriptfile,   dirstr,   stepindx,
                                   wlaslist,   wenvlist,   Ilist,   CEPlist,
                                   tstartlist,  polthetalist,  polphaselist,
                                   timestep,   measurementtimestep,
                                   measurementtime):
    #call ellipticalpulse_list to write N elliptically polarized pulses as 2N
    #linearly polarized pulses.  Then call multiplepulses to set up input files

    tmpwlaslist, tmpwenvlist, tmpIlist, tmpCEPlist, tmptstartlist, tmppulsethetalist =\
            ellipticalpulse_list(wlaslist, wenvlist, Ilist, polthetalist,
                                 polphaselist, CEPlist, tstartlist)
    
    multiplepulses(masterscriptfile,  dirstr,  stepindx,  tmpwlaslist,
                   tmpwenvlist,  tmpIlist,  tmpCEPlist,  tmptstartlist,
                   tmppulsethetalist,  timestep,  measurementtimestep,
                   measurementtime)
...
def streakingcalculation(resultdir, calcindx, txuvoffsetlist, CEPlist,
                         wenvlist,  wlaslist,  Ilist,  polthetalist,
                         polphaselist,  timestep,  tmeasure, 
                         measurementtimestep):
    ...
    multiplepulses_ellipticalpulse(masterscriptfile, dirstr, 1, wlaslist,
                                   wenvlist, Ilist, CEPlist[CEPindx],
                                   tstartlist, polthetalist, polphaselist,
                                   timestep, measurementtimestep, tmeasure)
...
def fullcalculation(resultdir,calcindx,deltatlist,CEPlist,wenvlist,wlaslist,\
    ...
    initialrelaxation(masterscriptfile,dirstr)
    multiplepulses(masterscriptfile,dirstr,1,wlaslist,wenvlist,Ilist,CEPlist,tstartlist,timestep,measurementtimestep,tmeasure)
...
Ilist=[I1,I2]
polthetalist=[pi/4, pi/4]
polphaselist=[pi/2, pi/2]
...
for calcindx in range(ncalcs):
    streakingcalculation(resultdir, calcindx, txuvoffsetlist,  CEPlist,
                         wenvlist,  wlaslist,  Ilist,  polthetalist,
                         polphaselist,  timestep,  tmeasure, 
                         measurementtimestep)

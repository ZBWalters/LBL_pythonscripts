...
class pulse:
    def __init__(self, I0=1.e-4, w0=.0565, wenv=.014, CEPtheta=0, jtheta=0,
                 ...
                 tcenter=0, tstart=None , pulsetype=2, chirp=0, ramp=0, longstep=0):
...
#        print("self.vpol2\t"+str(self.vpol2))
    ...
    def initMCTDHF(self):
        ...
        strtphase1=mod(strtphase+self.jphiz, 2*pi)
        strtphase2=strtphase
...
def pulselisttostring(pulselist):
    retstr=''
    for i in range(len(pulselist)):
        retstr+=pulselist[i].tostring()+"\t"
    return retstr+"\n"

def printloopkey(resultdir, loopnamelist, looplist, filename='loopkey.txt'):
    tmpfile=open(resultdir+filename,'w')
    tmpfile.write("arrays looped over in this calculation (descending order of significance)\n")
...
paramindx=0
pulsearray=[]
loopnamelist=['dtarray','xuvphasearray','IRphasearray']
looplist=[dtarray, xuvphasearray, IRphasearray]
printloopkey(resultdir,loopnamelist, looplist)
for i,j,k in iterproduct(range(ndt), range(nxuvphase), range(nIRphase)):
    dt=dtarray[i]
    xuvphase=xuvphasearray[j]*pi
    IRphase=IRphasearray[k]*pi
    pulselist=[pulse(I0=Iir, w0=wir, wenv=IRwenv, CEPtheta=IRphase, tstart=0),
               pulse(I0=Ixuv, w0=wxuv, wenv=XUVwenv, CEPtheta=xuvphase, tcenter=IRtmid_atomic+dt)]

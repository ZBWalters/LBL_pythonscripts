...
def fullcalculation(resultdir,calcindx,deltatlist,CEPlist,wenvlist,wlaslist,\
    ...
    [deltat1,deltat2,deltat3]=deltatlist
    ...
    measurmentinterval(masterscriptfile,dirstr,7,tmeasure,timestep,measurementtimestep)
...
shutil.copy("./twoDspectruminputs_phasematching_multiprocessing.py",resultdir)
...
tmeasure=T1max#256#total interval to propagate when calculating dipole
measurementtimestep=dt1#4.#interval between calculating dipole
...
w2=(5.3+10.6)/(2*Hrt)#.55#1.3#frequency of driving laser
w=w2/8.#((2*pi)/(1000/aut))#2./Hrt#((2*pi)/(1000/aut))#frequency of sin^2 envelope
exciteintensity=1.e-2
